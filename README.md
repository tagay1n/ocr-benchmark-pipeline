...
Verification extraction reuses identical full-resolution PNG crops across validators and retries through an in-memory least-recently-used cache, bounded to 128 entries and 64 MiB of encoded PNG data. Oversized crops are used without caching. Source path/file metadata, bbox coordinates, and effective rotation determine cache identity. Changing any of them regenerates the crop; restarting clears the cache. Review thumbnails remain separate, and extraction resolution and model thinking settings are unchanged.

Gemini key selection skips keys recorded as daily-quota exhausted for the selected model, shuffles the remaining available keys, then uses the first shuffled key. Daily exhaustion is merged and atomically persisted to `_artifacts/gemini_usage.json` immediately after each response, grouped by model. It does not expire with the date: the legacy `quota_day` field is informational only. Remove this JSON file (or the configured `GEMINI_USAGE_PATH`) to reset exhaustion manually. Prefer stopping extraction first so an in-flight quota response does not immediately record exhaustion again, then resume. This clears local bookkeeping, not Gemini's server-side limits. The file stores SHA-256 key fingerprints rather than API-key values and automatically migrates legacy raw-key entries when read. Per-minute rate limits are not recorded as daily exhaustion.
...
- Quote mark fidelity policy:
  - Preserve quotation marks, apostrophes, primes, modifier letters, accents, and guillemets exactly as printed.
  - Examples: `‘text’`, `ʼtextʼ`, `′text′`, `“text”`, `„text“`, `‹text›`, and `«text»` remain unchanged.
  - Store OCR and reviewer content in Unicode NFC form, but do not fold distinct punctuation into ASCII quotes.
  - Any quote-tolerant comparison belongs in an explicitly documented benchmark scoring profile applied equally to references and predictions; it must not alter stored content.
